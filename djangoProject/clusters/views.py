...
from clusters.models import Food, Polyclinic, Clusters, Cluster_name
...
def selected_cluster(request):
    if request.method == "GET":
        ...
        objects = Clusters.ClustersBundels.get_selection(id_cluster, 10)
...
def update_name_cluster(request):
    if request.method == "GET":
        ...
        objects = Cluster_name.Name.objects.filter(cluster=cluster).all()
        if len(objects) == 0:
